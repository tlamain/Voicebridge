...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const t = getNamespace(locale as Locale, "who-its-for");
...
export default async function WhoItsForPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const t = getNamespace(l, "who-its-for");
    ...
    <div className="py-16 space-y-20">
      ...
      <div className="space-y-8">
        {audienceConfigs.map((config) => {
          const audience = t.audiences.find((item) => item.id === config.id);
          if (!audience) return null;

          return (
            <ScrollReveal key={audience.id}>
                ...
                <div className="p-8 sm:p-10">
                  <div className="flex flex-col lg:flex-row gap-8">
                    <div className="lg:w-2/5 space-y-4">
                      <div className={`inline-flex h-14 w-14 items-center justify-center rounded-2xl ${config.bgColor} text-slate-700`}>
                        {config.icon}
                      </div>
                    ...
                    <div className="lg:w-3/5">
                      <div className={`rounded-2xl ${config.bgColor} p-6`}>
                        <p className="text-sm font-semibold text-slate-700 mb-4 uppercase tracking-wider">{t.benefitsHeading}</p>
