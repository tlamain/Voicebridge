...
│   ├── BoardButtonCell.tsx         # Grid cell button
...
│   ├── SlotEditorModal.tsx         # Slot editing
│   ├── LinkEditorModal.tsx         # Category link editing
...
### Database Architecture (WatermelonDB v23)

#### Core Tables

| Table | Purpose | Key Fields |
|-------|---------|------------|
| **symbols** | Vocabulary items | label, emoji, image_uri, language, category_id, position, motor_zone, introduction_level, source_pack_id, fitz_category, concept_key |
| **symbol_translations** | Multi-language symbol data | symbol_id, language, label, spoken_text, infinitive |
...
| **core_fringe_slots** | Cell positions within a page | page_id, row_index, column_index, is_pinned, symbol_id, is_category_link, link_to_page_id, link_image_uri, is_visible |
...
| **elevenlabs** | ElevenLabs API (`eleven_flash_v2_5` model) | Requires API key, configurable voice ID and speed, falls back to device on failure |
...
class VocabularyPackService {
  private static instance: VocabularyPackService;

  static getInstance(): VocabularyPackService {
    if (!this.instance) {
      this.instance = new VocabularyPackService();
    }
...
- **Edit Mode**: Add/move/delete symbols, toggle pin state, manage pages, edit category links
...
| **Ref-based Stable References** | `pinnedSlotsRef` in Core-Fringe mode prevents unnecessary React.memo re-renders |
...
│   ├── schema/         # WatermelonDB schema (v23)
...
4. **Data Persistence**: WatermelonDB with 23 migrations for schema evolution
...
12. **Multi-Provider TTS**: Device speech and ElevenLabs cloud TTS with automatic fallback
13. **Portable Backups**: Selective .vbaac backup/restore with embedded base64 images for cross-device portability
